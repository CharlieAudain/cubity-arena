...
export default function App() {
  ...
  // 1. LISTEN: Authentication & Database Connection
  useEffect(() => {
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-950 text-blue-500 gap-4">
        ...
        <span className="text-slate-400 text-sm font-mono animate-pulse">CONNECTING TO DATABASE...</span>
      </div>
    );
  }
      ...
      <header className="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-4 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-lg font-bold tracking-tight leading-none text-white">CUBITY</span>
            <span className="text-[10px] text-blue-400 font-bold tracking-wider">ARENA</span>
          </div>
        </div>

        <div className="flex items-center gap-3 md:gap-6">
          {/* Online Counter (Fake for now, easier to implement later) */}
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-900 rounded-full border border-white/5">
            ...
            <span className="text-xs font-medium text-slate-400 tabular-nums">{onlineCount.toLocaleString()} Online</span>
          </div>
          ...
          {user ? (
            ...
            >
              <div className="flex flex-col items-end mr-1">
                ...
                <span className="text-xs font-bold text-slate-200">{userData?.rank || "Loading..."}</span>
                <span className="text-[10px] text-slate-500 font-mono">ELO {userData?.elo || "---"}</span>
              </div>
              <div className="relative w-8 h-8 rounded-full bg-gradient-to-br from-slate-700 to-slate-800 border border-slate-600 flex items-center justify-center text-xs font-bold ring-2 ring-transparent group-hover:ring-red-500/50 transition-all overflow-hidden">
                <span className="group-hover:opacity-0 transition-opacity">{user.uid.slice(0, 1).toUpperCase()}</span>
                <LogOut className="w-4 h-4 text-red-500 absolute opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </button>
          ) : (
            <button 
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <div className="flex items-center gap-2 mb-2">
                   {user && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-500 border border-amber-500/20">SEASON 4</span>}
                   <span className="flex items-center gap-1 text-xs font-medium text-slate-400">
                    <Flame className="w-3 h-3 text-orange-500" /> Daily Streak: {userData?.streak || '0'}
                   </span>
                </div>
                <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2 tracking-tight">
                  {user ? "Ready to Race?" : "Welcome to the Arena"}
                </h1>
                <p className="text-slate-400 max-w-md text-sm sm:text-base leading-relaxed">
                  Global Daily Scramble #42 is live.
                </p>
              </div>
