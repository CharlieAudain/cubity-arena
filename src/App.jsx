...
const applyCubeMove = (state, move, type) => {
  ...
  const getCol = (faceIdx, colIdx) => {
    let col = [];
    for (let i = 0; i < size; i++)
      col.push(newState[faceIdx][i * size + colIdx]);
    return col;
  };
  const setCol = (faceIdx, colIdx, data) => {
    for (let i = 0; i < size; i++)
      newState[faceIdx][i * size + colIdx] = data[i];
  };
  ...
  for (let t = 0; t < turns; t++) {
    if (base === "U") {
      newState[0] = isPrime
        ? rotateFaceCounter(newState[0], size)
        : rotateFace(newState[0], size);
      ...
      newState[3] = isPrime
        ? rotateFaceCounter(newState[3], size)
        : rotateFace(newState[3], size);
      let sides = [
        getRow(2, size - 1),
        getRow(1, size - 1),
        getRow(5, size - 1),
      ...
      sides = cycle(sides, !isPrime);
      setRow(2, size - 1, sides[0]);
      setRow(1, size - 1, sides[1]);
      setRow(5, size - 1, sides[2]);
      setRow(4, size - 1, sides[3]);
    } else if (base === "F") {
      newState[2] = isPrime
        ? rotateFaceCounter(newState[2], size)
        : rotateFace(newState[2], size);
      ...
      newState[5] = isPrime
        ? rotateFaceCounter(newState[5], size)
        : rotateFace(newState[5], size);
      let uRow = getRow(0, 0);
      let lCol = getCol(4, 0);
      ...
      newState[1] = isPrime
        ? rotateFaceCounter(newState[1], size)
        : rotateFace(newState[1], size);
      ...
      newState[4] = isPrime
        ? rotateFaceCounter(newState[4], size)
        : rotateFace(newState[4], size);
      let uCol = getCol(0, 0);
      let fCol = getCol(2, 0);
      let dCol = getCol(3, 0);
...
const calculateBestAverage = (solves, size) => {
  ...
  // Scan through all solves to find best window
...
const StatsView = ({ userId }) => {
  ...
  const [statsMode, setStatsMode] = useState("current"); // 'current' or 'best'
  ...
  useEffect(() => {
    let filtered = solves;
    if (filterType !== "all") {
      filtered = solves.filter(
        (s) => s.type === filterType || (filterType === "3x3" && !s.type)
      );
    }
    setFilteredSolves(filtered.slice(0, viewLimit));
  }, [solves, filterType, viewLimit]);

  // Dynamic Stats Calculation based on Mode
  const displayAo5 =
    statsMode === "current"
      ? calculateAverage(filteredSolves, 5)
      : calculateBestAverage(
          solves.filter((s) => filterType === "all" || s.type === filterType),
          5
        );
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center gap-6 text-sm text-slate-400">
                ...
                <div className="flex items-center gap-2 text-slate-500">
                  <Box className="w-4 h-4" /> {selectedSolve.type || "3x3"}
                </div>
              </div>
