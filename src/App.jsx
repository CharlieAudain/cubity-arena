...
import { 
  Trophy, Users, Zap, Settings, History, Timer, 
  LogIn, Activity, Bluetooth, Flame, LogOut, Edit2, Save, Swords
} from 'lucide-react';
import { 
  onAuthStateChanged, 
  signInAnonymously, 
...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.message.includes('credential-already-in-use') 
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-24 md:pb-0">
      ...
      <header className="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-4">
        ...
        <div>
          {user ? (
            <button onClick={handleLogout} className="flex items-center gap-3 bg-slate-900 border border-white/10 px-3 py-1 rounded-full hover:bg-slate-800">
              ...
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${user.isAnonymous ? 'bg-slate-700' : 'bg-green-600'}`}>
                {userData?.displayName?.[0]?.toUpperCase() || "G"}
              </div>
            </button>
          ) : (
            <button onClick={handleGoogleLogin} className="flex items-center gap-2 text-sm font-bold text-slate-900 bg-white hover:bg-slate-200 px-4 py-2 rounded-lg">
            ...
            {user && userData ? (
              <div className="mb-4">
                ...
                <div className="flex items-center gap-3 mb-6">
                  {isEditingName ? (
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <button onClick={saveName} className="p-2 bg-blue-600 rounded-lg text-white hover:bg-blue-500"><Save className="w-5 h-5"/></button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-3 group">
                      <h1 className="text-3xl font-bold text-white">{userData.displayName}</h1>
                      <button onClick={() => setIsEditingName(true)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-500 hover:text-white transition-opacity">
                ...
                <button 
                  ...
                  className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-xl font-bold shadow-lg shadow-blue-500/25 transition-all active:scale-95 flex items-center justify-center gap-3 w-full sm:w-auto animate-pulse hover:animate-none"
                >
                  <Swords className="w-6 h-6 fill-current" />
                  <span className="tracking-wide">FIND MATCH</span>
                </button>
                <p className="mt-3 text-xs text-slate-500 font-mono">EST. WAIT: 4s • 1,240 ONLINE</p>
              </div>
            ) : (
              <>
                <h1 className="text-3xl font-bold mb-2">Welcome to the Arena</h1>
                <p className="text-slate-400 mb-6">Compete in the global daily scramble.</p>
              </>
            )}

            {!user ? (
              <div className="flex flex-col sm:flex-row gap-3">
                 <button onClick={handleGuestLogin} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2">
                  <Zap className="w-5 h-5" /> Start Guest Session
                </button>
                <button onClick={handleGoogleLogin} className="bg-slate-800 hover:bg-slate-700 text-white px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 border border-white/10">
               ...
               user.isAnonymous && (
                <div className="mt-6 p-4 bg-blue-900/20 border border-blue-500/20 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                   <div className="text-sm text-blue-200">
                      <strong>You are a Guest.</strong> Link your Google account to save your stats permanently.
                   </div>
                   <button onClick={handleGoogleLogin} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold whitespace-nowrap">
                     Link Google Account
                   </button>
                </div>
               )
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="p-5 bg-slate-900/50 rounded-xl border border-white/5 hover:border-purple-500/30 hover:bg-slate-900 transition-all group cursor-pointer relative overflow-hidden">
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <span className="text-xs font-mono text-slate-500 bg-slate-800 px-2 py-1 rounded">RES: 14h</span>
            </div>
            <h3 className="font-bold text-lg mb-1 text-slate-200 group-hover:text-white">Daily Scramble</h3>
            <p className="text-sm text-slate-500 group-hover:text-slate-400">Compete against the world on the exact same shuffle.</p>
          </div>
          <div className="p-5 bg-slate-900/50 rounded-xl border border-white/5 hover:border-emerald-500/30 hover:bg-slate-900 transition-all group cursor-pointer relative overflow-hidden">
             <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <span className="text-xs font-mono text-slate-500 bg-slate-800 px-2 py-1 rounded">LOCAL</span>
            </div>
            <h3 className="font-bold text-lg mb-1 text-slate-200 group-hover:text-white">Solo Practice</h3>
            <p className="text-sm text-slate-500 group-hover:text-slate-400">Standard timer with Ao5/Ao12 tracking.</p>
          </div>
           <div className="p-5 bg-slate-900/50 rounded-xl border border-white/5 hover:border-orange-500/30 hover:bg-slate-900 transition-all group cursor-pointer relative overflow-hidden">
             <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <span className="text-xs font-mono text-slate-500 bg-slate-800 px-2 py-1 rounded">OFFLINE</span>
            </div>
            <h3 className="font-bold text-lg mb-1 text-slate-200 group-hover:text-white">Smart Cube</h3>
            <p className="text-sm text-slate-500 group-hover:text-slate-400">Connect your GAN or GoCube via Bluetooth.</p>
          </div>
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 w-full bg-slate-950/90 backdrop-blur-xl border-t border-white/5 pb-safe px-6 py-2 flex justify-between items-center z-50 md:hidden h-20">
        <NavIcon icon={Trophy} label="Arena" active={activeTab === 'arena'} onClick={() => setActiveTab('arena')} />
...
function NavIcon({ icon: Icon, label, active, onClick }) {
  return (
    <button 
      onClick={onClick}
      className={`relative flex flex-col items-center gap-1.5 p-2 transition-all duration-300 ${active ? 'text-blue-500 -translate-y-1' : 'text-slate-500 hover:text-slate-300'}`}
    >
      <div className={`absolute inset-0 bg-blue-500/20 blur-xl rounded-full transition-opacity duration-300 ${active ? 'opacity-100' : 'opacity-0'}`}></div>
      <Icon className={`w-6 h-6 relative z-10 ${active ? 'fill-blue-500/20' : ''}`} />
      <span className="text-[10px] font-bold relative z-10">{label}</span>
      {active && <div className="absolute -bottom-2 w-1 h-1 rounded-full bg-blue-500"></div>}
    </button>
  );
}